...
exports.getLighthouseGraphByPwaId = function(pwaId) {
  return new Promise((resolve, reject) => {
    const query = ds.createQuery(ENTITY_NAME)
      .filter('pwaId', '=', parseInt(pwaId, 10));
    ds.runQuery(query, (err, lighthouseGraphData) => {
      if (err) {
        return reject(err);
      }
      if (lighthouseGraphData.length === 0) {
        return resolve(null);
      }

      // Graph data uses this format:
      // https://developers.google.com/chart/interactive/docs/reference#dataparam
      let data = {};
      data.cols = [{label: 'Date', type: 'date'}, {label: 'Score', type: 'number'}];
      data.rows = [];
      lighthouseGraphData.forEach(lighthouse => {
        let date = new Date(Date.parse(lighthouse.data.date));
        data.rows.push(
          {c: [{v:
