...
function list(kind, offset, limit, sort) {
  return runQuery(createQuery(kind, offset, limit, sort));
}
 ...
 * @return {query}
 */
function createQuery(kind, offset, limit, sort) {
...
function update(kind, id, data) {
  return new Promise((resolve, reject) => {
    ...
    const entity = {
      key: key,
      data: toDatastore(data, ['description', '_manifest', '_lighthouseJson', 'lighthouseInfo'])
    };
