...
/**
 * Finds a PWA by an arbitrary equality filter.
 *
 * @param {string} field to search by
 * @param {string} value that the field must equals
 * @return {Pwa} the PWA from DB
 */
function findByFilter(field, value) {
  return new Promise((resolve, reject) => {
    const query = ds.createQuery(ENTITY_NAME).filter(field, value);
...
exports.findByManifestUrl = function(manifestUrl) {
  return findByFilter('manifestUrl', manifestUrl);
};

/**
 * Finds a PWA by its encodedStartUrl from DB.
 *
 * @param {string} encodedStartUrl based on the PWA's manifest
 * @return {Pwa|null} the PWA from DB, or null if not found
 */
exports.findByEncodedStartUrl = function(encodedStartUrl) {
  return findByFilter('encodedStartUrl', encodedStartUrl);
};

/**
 * Finds a PWA by any of its previousEncodedStartUrl from DB.
 *
 * @param {string} previousEncodedStartUrl based on the PWA's manifest
 * @return {Pwa|null} the PWA from DB, or null if not found
 */
exports.findByPreviousEncodedStartUrl = function(previousEncodedStartUrl) {
  return findByFilter('previousEncodedStartUrl', previousEncodedStartUrl);
};
