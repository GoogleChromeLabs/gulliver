    ...
    .then(manifest => {
      return this.findByManifestUrl(pwa.manifestUrl)
        ...
        .then(result => {
          ...
          Promise.all([
            this.updatePwaMetadataDescription(result.pwa),
            this.updatePwaIcon(result.pwa),
            this.updatePwaLighthouseInfo(result.pwa)])
          .then(_ => {
            this.savePwa(result.pwa)
              .then(() => {
                if (result.created) {
                  this.sendNewAppNotification(result.pwa.id);
                }
              });
          });
          return result.pwa;
...
exports.sendNewAppNotification = function(pwaid) {
  console.log('Sending Notification for ', pwaid);
  return this.find(pwaid)
    .then(pwa => {
      if (!pwa) {
        return Promise.reject('PWA not found. ' + pwaid);
      }
