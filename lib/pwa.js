...
/**
 * Persists `pwa` (including the associated Lighthouse report). Promise rejects
 * if the `manifestUrl` or `user.id` is missing, otherwise it resolves to an
 * updated `pwa` object. (`pwa` is likely to be modified/populated in place.)
 *
 * @param {Pwa} pwa
 * @returns {Promise<Pwa>}
 */
...
exports.save = function(pwa) {
  ...
  if (!(pwa instanceof Pwa)) {
    return Promise.reject('pwa is not an instance of Pwa');
  }

  if (!pwa.manifestUrl) {
    return Promise.reject('Missing manifestUrl');
  }

  if (!(url.format(pwa.manifestUrl).startsWith('http://') ||
        url.format(pwa.manifestUrl).startsWith('https://'))) {
    return Promise.reject(`pwa.manifestUrl [${pwa.manifestUrl}] is not a valid URL`);
  }
