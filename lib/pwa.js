 ...
 *    c) Delete modified PWAs from memcached
 *
  ...
  return promiseSequential.all([
    ...
    savedPwa => {
      ...
      if (savedPwa.isNew()) {
        ...
        setTimeout(_ => this.getPwaPerformanceInfo(savedPwa), 3 * 60 * 1000);
      } else {
        this.getPwaPerformanceInfo(savedPwa);
      }
      // Delete modified PWA from memcached.
      const url = '/pwas/' + pwa.id;
      cache.del(url);
      cache.del(url + '?contentOnly=true');
      return savedPwa;
    }
  ]);
};
