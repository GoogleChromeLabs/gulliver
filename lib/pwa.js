 ...
 * Steps:
 *  1) Validate PWA
 *  2) Update Pwa's Manifest and save
 *  3) Update PWA's MetadataDescription & Icon and save
 *  4) (in background) Update Lighthouse & send notifications for new PWAs.
 *
 * @param {Pwa} pwa to update
 * @return {Pwa} the updated PWA
 */
exports.process = function(pwa) {
  const valid = this.validate(pwa);
  if (!valid) {
  ...
  return this.updatePwaManifest(pwa)
  .then(pwa => {
    return Promise.all([
      this.updatePwaMetadataDescription(pwa),
      this.updatePwaIcon(pwa)])
    .then(_ => {
      // finish up Lighthouse info and notification in background
      this.updatePwaLighthouseInfo(pwa).then(pwa => {
        this.sendNewAppNotification(pwa);
      });
 ...
 * @return {String || true} error code or true if valid
 */
exports.validate = function(pwa) {
  if (!pwa || !(pwa instanceof Pwa)) {
    return E_NOT_A_PWA;
  }
  if (!pwa.manifestUrl) {
    return E_MANIFEST_URL_MISSING;
  }
  if (!(url.format(pwa.manifestUrl).startsWith('http://') ||
        url.format(pwa.manifestUrl).startsWith('https://'))) {
    return E_MANIFEST_INVALID_URL;
  }
  if (!pwa.user || !pwa.user.id) {
    return E_MISING_USER_INFORMATION;
  }
  return true;
};
...
/**
 * Update manifest and save a PWA to the DB.
 *
 * Steps:
 ...
 * @return {Promise<JSON>} the result of the push notification
 */
exports.sendNewAppNotification = function(pwa) {
  if (pwa.created === pwa.updated) {
...
/**
 * Updates the Lighthouse information, sets the lighthouseScore for a PWA and saves.
 *
 * @param {Pwa} the PWA to update
 * @return {Promise<Pwa>} with the updated PWA
 */
