...
// Set slugified name to use for user friendly URL
exports.slugify = function(pwa) {
  const slugifiedName = slug(pwa.manifest.name);
  const slugifiedDescription = slug(pwa.description);
  return new Promise((resolve, reject) => {
    this.findBySlugified(slugifiedName)
      .then(pwaBySlug => {
        if (slugifiedName && (!pwaBySlug || pwaBySlug.id === pwa.id)) {
          pwa.slugified = slugifiedName;
          resolve(pwa);
        } else {
          this.findBySlugified(slugifiedDescription)
            .then(pwaBySlug => {
              if (slugifiedDescription && (!pwaBySlug || pwaBySlug.id === pwa.id)) {
                pwa.slugified = slugifiedDescription;
              } else {
                pwa.slugified = pwa.id;
              }
              resolve(pwa);
            });
        }
    ...
    .then(manifest => {
      return this.findByManifestUrl(pwa.manifestUrl)
        ...
        .then(metaDescription => {
          ...
          return this.slugify(pwa);
        })
        .then(slugifiedPwa => {
          return db.update(ENTITY_NAME, pwa.id, slugifiedPwa);
        })
        .then(savedPwa => {
