...
describe('controllers.api.favorite-pwa', () => {
  ...
  describe('GET /api/favorite-pwa/', () => {
    ...
    it('respond with 403 if missing user idToken', done => {
      // /api/ is part of the router, we need to start from /favorite-pwa/
      request(app)
        .get('/favorite-pwa')
        .expect('Content-Type', /json/)
        .expect(403).should.be.fulfilled.then(res => {
          assert.equal(res.body, '403 Forbidden, missing user idToken');
          done();
        });
    });
    ...
    it('respond with 200 if Favorite PWA exist', done => {
      ...
      // /api/ is part of the router, we need to start from /favorite-pwa/
      request(app)
        .get('/favorite-pwa?idToken=1234567890')
        .expect('Content-Type', /json/)
  ...
  describe('GET /api/favorite-pwa/:pwaId', () => {
    ...
    it('respond with 200 if isFavorite exist', done => {
      simpleMock.mock(libFavoritePwa, 'isFavorite').resolveWith('list of favorite pwas');
      simpleMock.mock(verifyIdToken, 'verifyIdToken').resolveWith(testGoogleLogin);
      // /api/ is part of the router, we need to start from /favorite-pwa/
      request(app)
        .get('/favorite-pwa/1234567?idToken=1234567890')
        .expect('Content-Type', /json/)
        .expect(200).should.be.fulfilled.then(res => {
          assert.equal(res.body, 'list of favorite pwas');
          assert.equal(libFavoritePwa.isFavorite.callCount, 1);
          assert.equal(libFavoritePwa.isFavorite.lastCall.args[0],
            '1234567');
          assert.equal(libFavoritePwa.isFavorite.lastCall.args[1],
            '01b307acba4f54f55aafc33bb06bbbf6ca803e9a');
          done();
        });
    });

    it('respond with 404 if isFavorite does not exist', done => {
      simpleMock.mock(libFavoritePwa, 'isFavorite').resolveWith(null);
      simpleMock.mock(verifyIdToken, 'verifyIdToken').resolveWith(testGoogleLogin);
      // /api/ is part of the router, we need to start from /favorite-pwa/
      request(app)
        .get('/favorite-pwa/1234567?idToken=1234567890')
